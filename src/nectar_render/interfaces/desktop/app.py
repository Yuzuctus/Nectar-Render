...
from ...adapters.rendering.markdown_pipeline import invalidate_image_index_cache
...
class NectarRenderApp:
    ...
    def _build_ui(self) -> None:
        ...
        left_panel = self._build_scrollable_panel(container)
        self._build_panels(left_panel)
    ...
    def _build_scrollable_panel(self, container: ttk.Frame) -> ttk.Frame:
        ...
        left_panel.bind("<Configure>", self._sync_left_scrollregion)
        self.left_canvas.bind("<Configure>", self._sync_left_width)
        return left_panel

    def _build_panels(self, left_panel: ttk.Frame) -> None:
        self._build_basic_panel(left_panel)
        self._build_typography_panel(left_panel)
        self._build_margins_panel(left_panel)
        self._build_advanced_panel(left_panel)

    def _build_basic_panel(self, left_panel: ttk.Frame) -> None:
    ...
    def _build_typography_panel(self, left_panel: ttk.Frame) -> None:
        self._typography_panel = TypographyPanel(
            ...
            on_font_commit=self._on_font_commit,
        )
    ...
    def _build_margins_panel(self, left_panel: ttk.Frame) -> None:
        MarginsPanel(
            left_panel,
            margin_top_var=self.margin_top_var,
            margin_right_var=self.margin_right_var,
    ...
    def _build_advanced_panel(self, left_panel: ttk.Frame) -> None:
        ...
        self._build_advanced_typography_panel()
        self._build_code_panel()
        self._build_table_panel()
        self._build_footnote_panel()

    def _build_advanced_typography_panel(self) -> None:
    ...
    def _build_code_panel(self) -> None:
    ...
    def _build_footnote_panel(self) -> None:
        FootnotePanel(
            self.advanced_panel,
            include_notes_var=self.include_notes_var,
            footnote_size_var=self.footnote_size_var,
    ...
    def _update_left_scroll_state(self) -> None:
    ...
    def _sync_left_scrollregion(self, _event: tk.Event[tk.Misc]) -> None:
        self.left_canvas.configure(scrollregion=self.left_canvas.bbox("all"))
        self._update_left_scroll_state()

    def _sync_left_width(self, event: tk.Event[tk.Misc]) -> None:
        self.left_canvas.itemconfigure(self.left_canvas_window, width=event.width)
        self.root.after_idle(self._update_left_scroll_state)

    def _on_font_commit(self, _value: str) -> None:
    ...
    def _capture_app_state(self) -> dict[str, object]:
        return capture_tk_state(self._tk_var_dict())

    def _collect_style(self):
        return collect_style_from_tk_state(
            self._tk_var_dict(),
            base_style=self.config.style,
        )

    def _collect_export(self):
        return collect_export_options(
            output_format=self.format_var.get(),
            page_size=self.page_size_var.get(),
            compression_enabled=bool(self.pdf_compression_enabled_var.get()),
            compression_profile=self.pdf_compression_profile_var.get(),
    ...
    def _detect_markdown_max_heading_level_from_text(markdown_text: str) -> int:
        return detect_markdown_max_heading_level_from_text(markdown_text)
